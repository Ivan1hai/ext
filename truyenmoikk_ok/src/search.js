function execute(key, page) {
	...
	// URL format: https://truyenmoiyy.com/tim-kiem?tukhoa=key&trang=1
	let fetchUrl = "https://truyenmoiii.org/tim-kiem?tukhoa=" + key + "&trang=" + page;
	let doc = Http.get(fetchUrl).html();

	if (!doc) return Response.error("Không thể tải trang tìm kiếm");
	...
	if (nextUrl) {
		let match = nextUrl.match(/trang-(\d+)/) || nextUrl.match(/trang=(\d+)/);
