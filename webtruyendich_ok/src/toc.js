...
function execute(url) {
    ...
    // Extract numeric novel_id from javascript tags
    let novelId = "";
    doc.select("script").forEach(script => {
        let text = script.html() || "";
        let idMatch = text.match(/novel_id\s*=\s*['"]?(\d+)['"]?/);
        if (idMatch) {
            novelId = idMatch[1];
        }
    });
    ...
    for (let source of sources) {
        try {
            let foundChaptersForSource = false;
                ...
                while (hasNext && page <= 10) { // Load up to 20k chapters max
                    ...
                    if (json && json.data && Array.isArray(json.data)) {
                        ...
                        json.data.forEach(item => {
                            ...
                            const chapterUrl = BASE_URL + "/truyen/" + slug + "/" + source + "/" + chapterSlug;
                            pageChapters.push({
                                name: sources.length > 1 ? title + " (" + source + ")" : title,
                                url: chapterUrl,
                                host: BASE_URL
                            });
                        });
                        ...
                        if (pageChapters.length > 0) {
                            ...
                            foundChaptersForSource = true;
                        }

                        if (json.pagination && json.pagination.has_next) {
                            page++;
                        } else {
                            hasNext = false;
                        }
            ...
            if (foundChaptersForSource) {
                break; // Found chapters for this source, move down to sort/return
            }

            // Try different possible endpoints for older structure or fallback
            const endpoints = [
    ...
    // Remove duplicates - keep API order (already correct)
